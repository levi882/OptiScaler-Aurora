...
两个面板首次显示时默认展开。“帧生成”区域的 FG Input、FG Output 和倍率设置下方直接显示共用的“强制动态 MFG”“DMFG 目标帧数”和应用/重置按钮，随后才是 RTX 20/30 与 RTX 40 面板。未检测到游戏 Streamline 插件报告支持动态 MFG 时，控件仍显示，但不可操作。RTX 20/30 的实际倍率仍受 SM86 `MaxGeneratedFrames` 与游戏插件上限约束；FPS Target 只在动态模式下生效。
